...
- **Events:** 이 가이드는 contract에서 이벤트를 발생시키는 방법을 설명하며, 이를 통해 온체인 활동을 추적할 수 있다. 이벤트에 대해 더 알아보려면 Sui에서 이벤트를 실제로 사용하는 방법을 다룬 [Using Events](/guides/developer/accessing-data/using-events.mdx)와 이벤트 구조 및 Move에서 이벤트를 발생시키는 방법을 설명하는 [Events in The Move Book](https://move-book.com/programmability/events.html)을 참조한다.
...
Sui에서 [programmable transaction block](/guides/developer/transactions/ptbs/prog-txn-blocks)(PTB)을 사용하려면 `Transaction`을 생성한다.
...
dApp에서 이벤트를 다루는 방법에 대해 더 알아보려면 [Using Events](/guides/developer/accessing-data/using-events.mdx)를 참조한다.
