...
- **Accessibility:** zkLogin은 [Sui’s cryptography agility](/concepts/cryptography/transaction-auth/signatures.mdx) 덕분에 여러 네이티브 Sui 서명 방식 중 하나이다. 이는 sponsored transactions와 multisig 같은 다른 Sui primitive 기능과 통합된다.
...
애플리케이션이나 지갑에 zkLogin을 통합하려는 빌더라면 [zkLogin integration guide](/guides/developer/cryptography/zklogin-integration/zklogin-integration.mdx)를 참고한다.
...
![zkLogin flow diagram](../../../../concepts/cryptography/images/zklogin-flow.png)
...
2. Salt Backup Service: 이는 고유 사용자별 salt를 반환하는 백엔드 서비스이다. salt를 유지하는 다른 전략은 [zkLogin Integration Guide](/guides/developer/cryptography/zklogin-integration/zklogin-integration.mdx)를 참고한다.
...
1. The address flag: zkLogin address를 위한 `zk_login_flag = 0x05`이다. 이는 [crypto agility](/concepts/cryptography/transaction-auth/signatures.mdx)에 정의된 서명 방식의 domain separator 역할을 한다.
...
3. `ext_eph_pk`: 임시 공개 키의 바이트 표현(`flag || eph_pk`)이다. 크기는 서명 방식 선택에 따라 달라진다(플래그로 표시되며 [Signatures](/concepts/cryptography/transaction-auth/signatures.mdx)에 정의됨).
