...
transaction은 보통 [programmable transaction blocks](/guides/developer/transactions/ptbs/building-ptb)(PTBs)를 사용해 구성되며, 구성된 뒤 사용자가 서명하고 온체인에서 실행되도록 제출한다.
...
이 가이드는 Multisig와 zkLogin에는 적용되지 않으므로, 지침은 각 전용 페이지([Multisig](/guides/developer/transactions/transaction-auth/multisig)와 [zkLogin](https://docs.sui.io/guides/developer/cryptography/zklogin-integration))를 참조한다.
...
- 여러 command를 연결한 `Transaction`을 만들어 transaction 데이터를 구성한다. 자세한 내용은 [Building Programmable Transaction Blocks](/guides/developer/transactions/ptbs/building-ptb)를 참조한다.
- SDK의 내장 gas 추정과 coin selection이 gas coin을 선택한다.

- [signature](/guides/developer/transactions/transaction-auth/auth-overview)를 생성하도록 transaction에 서명한다.
...
// $MNEMONICS는 wordlist에 있는 12/15/18/21/24개 단어를 가리키며, 예를 들어 "retire skin goose will hurry this field stadium drastic label husband venture cruel toe wire" 같은 형태이다. 자세한 내용은 [Keys and Addresses](/guides/developer/transactions/transaction-auth/auth-overview)를 참조한다.
...
자세한 내용은 [Building Programmable Transaction Blocks](./building-ptb.mdx)를 참조한다.
...
`$MNEMONICS`는 wordlist에 있는 12/15/18/21/24개 단어를 가리키며, 예를 들어 "retire skin goose will hurry this field stadium drastic label husband venture cruel toe wire" 같은 형태이다. 자세한 내용은 [Keys and Addresses](/guides/developer/transactions/transaction-auth/auth-overview)를 참조한다.
...
1. 이 가이드는 단일 private key로 서명하는 방법을 보여준다. 더 복잡한 signing policy를 설정하는 것이 적절할 때는 [Multisig](/guides/developer/transactions/transaction-auth/multisig.mdx)를 참조한다.
2. 마찬가지로 native zkLogin은 위 단계를 따르지 않으므로, zkLogin address를 파생하고 ephemeral key pair로 zkLogin signature를 생성하는 방법을 이해하려면 [the docs](/guides/developer/cryptography/zklogin-integration/)를 참조한다.
3. 이전 도구 대신 자체 signing mechanism을 구현하기로 했다면, 각 scheme에 대해 허용되는 signature specification은 [Signatures](/guides/developer/transactions/transaction-auth/auth-overview) 문서를 참조한다.
4. Flag는 signature scheme을 구분하는 1 byte이다. 지원되는 scheme과 해당 flag는 [Signatures](/guides/developer/transactions/transaction-auth/auth-overview)에서 확인한다.
5. `execute_transaction_block` endpoint는 signature 목록을 받으므로, sponsored transaction을 사용하지 않는 한 정확히 1개의 사용자 signature만 포함해야 하고, sponsored transaction을 쓰는 경우에는 gas object에 대한 두 번째 signature를 제공할 수 있다. 자세한 내용은 [Sponsored Transactions](/guides/developer/transactions/sponsor-txn.mdx)를 참조한다.