...
| Mutate contract state | instruction data를 포함한 transactions를 전송한다. Programs는 instructions를 처리하고 account 데이터를 수정한다. Type-safe 클라이언트 상호작용을 위해 Anchor의 IDL(Interface Definition Language)을 사용할 수 있다. | 런타임 [programmable transaction block](/guides/developer/transactions/ptbs/prog-txn-blocks) (PTB) 구성을 통해 transaction을 전송한다. |
...
Sui의 모든 ownership 유형은 [Object Ownership](/guides/developer/objects/object-ownership)을 참조하라.
...
PTB에 대한 자세한 내용은 [Programmable Transaction Blocks](/guides/developer/transactions/ptbs/prog-txn-blocks)를 참조하라.
