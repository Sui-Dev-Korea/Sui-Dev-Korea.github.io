...
Sui 네트워크는 [set of validators](/concepts/sui-architecture/consensus.mdx)에 의해 운영되며, 이들은 [transactions](/guides/developer/transactions/txn-overview.mdx)을 처리한다. 이들은 네트워크에 제출되고 처리된 어떤 transactions가 유효하고 [_consensus_](/concepts/sui-architecture/consensus.mdx)를 통해 final되어야 하는지에 대해 합의한다.
...
Move smart contract language는 audit과 verification을 쉽게 할 수 있도록 설계되었다. [introduction to smart contracts in Move](/concepts/sui-move-concepts)에 관심이 있을 수 있다.
