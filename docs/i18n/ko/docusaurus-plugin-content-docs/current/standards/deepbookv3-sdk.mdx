...
어떤 trade를 배치하기 전에 client에 balance manager address를 제공해야 한다. manager key는 client의 `BalanceManager` 인터페이스로 정의된 object를 가리킨다. [BalanceManager docs](./deepbookv3/contract-information/balance-manager.mdx). client로 balance manager를 초기화한다. balance manager를 생성하지 않으면 client가 대신 생성하도록 할 수 있지만, 그러면 사용자는 client를 다시 초기화해야 한다.
