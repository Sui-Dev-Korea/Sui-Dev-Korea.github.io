...
fund를 차입해 trading할 때, 최소 [risk ratio](./contract-information/risk-ratio.mdx)를 유지해야 한다(총 asset 대비 총 debt의 비율이다). position의 risk ratio가 **Liquidation Risk Ratio** 이하로 떨어지면, 누구나 position을 liquidation할 수 있다.
...
DeepBook Margin은 position을 자동으로 닫는 [TPSL orders](./contract-information/tpsl.mdx)를 지원한다:
