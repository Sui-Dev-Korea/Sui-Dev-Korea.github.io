...
Replay 도구를 사용하면 `--trace` 플래그를 전달하여 Move 디버거에서 사용할 수 있는 transaction 실행 추적을 생성할 수 있다. Move 디버거에 대한 자세한 내용은 [Move Debugger](../ide/debugger.mdx) 페이지를 참조한다. [gas profiling](./trace-analysis.mdx)과 같은 것은 다양한 [analyses on the transaction traces](./trace-analysis.mdx#profiling-a-transaction)로 수행할 수 있다.
